...
export function SupportContactClient({ locale, intakeProductId, initialTopic }: SupportContactClientProps) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      const response = await fetch('/api/front/inquiries', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        setFeedback({ tone: 'error', text: payload?.message ?? 'Unable to submit the support request right now.' });
        return;
      }

      const created = (await response.json()) as { id: string; redirectPath?: string };
      setReceipt(created);
