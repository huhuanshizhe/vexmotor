...
export function RegisterBusinessForm({ locale, initialEmail }: RegisterBusinessFormProps) {
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (!response.ok) {
        setFeedback(payload?.message ?? 'Unable to create the business account right now.');
        return;
      }
      ...
      const result = await signIn('credentials', {
        ...
        redirect: false,
        callbackUrl: payload?.redirectPath ?? '/account?pendingReview=1',
      });
      ...
      router.push(payload?.redirectPath ?? '/account?pendingReview=1');
      router.refresh();
    });
  }
