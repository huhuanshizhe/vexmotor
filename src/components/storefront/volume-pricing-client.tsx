  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (!response.ok) {
        ...
        setFeedback({ tone: 'error', text: payload?.message ?? 'Unable to submit the contract-pricing request right now.' });
        return;
      }

      const created = (await response.json()) as { redirectPath?: string };
