...
const shippingLanes = [
  {
    ...
    origin: 'Shanghai / Nanjing export hub',
...
export default async function SupportShippingPage() {
  ...
  const shippingLanes = commerceConfig.shippingMethods
    ...
    .map((method) => ({
      warehouse: method.code === 'warehouse-pickup' ? 'Factory pickup point' : 'CN Warehouse',
      carrier: method.name,
      eta: method.etaLabel,
      origin: method.code === 'warehouse-pickup' ? 'Warehouse appointment' : 'Shenzhen / Shanghai export hub',
      fit: method.note,
    }));
