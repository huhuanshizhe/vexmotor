...
export function AddressesClient({ initialAddresses }: { initialAddresses: Address[] }) {
  ...
  function submit() {
    startTransition(async () => {
      ...
      const response = await fetch(editingId ? `/api/front/addresses/${editingId}` : '/api/front/addresses', {
      ...
      const next = await fetch('/api/front/addresses', { cache: 'no-store' });
      const nextRows = (await next.json()) as Address[];
  ...
  function removeAddress(id: string) {
    startTransition(async () => {
      setMessage(null);
      const response = await fetch(`/api/front/addresses/${id}`, { method: 'DELETE' });
      if (!response.ok) {
