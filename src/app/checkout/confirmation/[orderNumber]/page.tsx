...
export default async function CheckoutConfirmationPage({ params }: { params: Promise<{ orderNumber: string }> }) {
  const cookieStore = await cookies();
  const { orderNumber } = await params;
  const order = await getGuestOrderDetailByNumber(orderNumber, cookieStore.get('guest_order_access_token')?.value ?? null);
